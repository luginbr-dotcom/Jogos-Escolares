...
function renderDado() {
  ...
  const especiais = {
    4:  {tipo:"bonus", emoji:"🎁", texto:"Bônus! Avance 2 casas.", efeito:2},
    7:  {tipo:"estrela", emoji:"⭐", texto:"Estrela da sorte! Jogue novamente.", efeito:"extra"},
    10: {tipo:"pergunta", emoji:"🧠", texto:"Desafio! Responda uma pergunta do professor.", efeito:"pergunta"},
    13: {tipo:"ponte", emoji:"🌉", texto:"Ponte! Atalho para frente.", efeito:4},
    16: {tipo:"energia", emoji:"⚡", texto:"Energia! Avance 3 casas.", efeito:3},
    19: {tipo:"volta", emoji:"🌧️", texto:"Imprevisto! Volte 2 casas.", efeito:-2},
    22: {tipo:"trofeu", emoji:"🏆", texto:"Conquista! Avance 3 casas.", efeito:3},
    26: {tipo:"pergunta", emoji:"📚", texto:"Desafio escolar! Responda antes de continuar.", efeito:"pergunta"},
    30: {tipo:"ponte", emoji:"🚀", texto:"Foguete! Salte 4 casas.", efeito:4},
  ...
  const casaHTML = casas.map(n => {
    ...
    return `<div class="casa${classe}" data-casa="${n}" style="grid-row:${linha(n)+1};grid-column:${coluna(n)+1}">
      ...
      ${especial ? `<small title="${especial.texto}">${especial.emoji}</small>` : "<small>•</small>"}
    </div>`;
  }).join("");
